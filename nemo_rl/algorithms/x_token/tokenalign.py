...
class TokenAligner(nn.Module):
    ...
    def build_vocab_partition(
            ...
            if 0 <= t_idx < teacher_vocab_size:
                if t_idx not in teacher_to_student_exact_map or xtoken_loss:
                    if t_idx in teacher_to_student_exact_map:
                        ...
                        if (
                            sorted_values[prev_student_token, 0]
                            >= sorted_values[s_idx, 0]
                        ):
                            continue
        ...
        uncommon_student = sorted(set(range(student_vocab_size)) - set(common_student))
    ...
    def _load_logits_projection_map(
        ...
        if use_sparse_format:
            ...
            #
            # (a) "Sparse-dict" format - dict whose keys are
            #     ``(student_id, teacher_id)`` tuples mapped to weights.
            #     Historically expected by this branch.
            #
            # (b) "Dense-tensor" format - dict with ``"indices"`` and
            #     ``"likelihoods"`` 2-D tensors of shape ``(V_s, top_k)``;
            #     produced by ``minimal_projection_generator.py`` and
            #     ``minimal_projection_via_multitoken.py``. We detect and
            #     convert to ``transformation_counts`` on the fly.
            ...
            if is_dense_tensor_format:
                ...
                # ``/ self.projection_matrix_multiplier`` divide below applies
                # the same normalization either way.
            ...
            if transformation_counts:
                ...
                # ``__init__`` sets ``self.sparse_transformation_matrix = None``
                # as a plain Python attribute, but ``register_buffer`` rejects
                # names that already exist on ``self``. Drop the placeholder
    ...
    def _canonical_token(token: str) -> str:
        ...
        unicode_fixes = {
            ...
            "Ã§": "ç",
            # French
            "Ã§": "ç",  # noqa: F601 - same-encoding duplicate kept for readability
            "Ã¨": "è",
            "Ã©": "é",  # noqa: F601 - same-encoding duplicate kept for readability
            "Ã«": "ë",
            "Ã®": "î",
            "Ã´": "ô",
            "Ã¹": "ù",
        ...
        special_token_map = {
            ...
            "": " ",  # End tokens
            "": " ",  # End tokens  # noqa: F601 - duplicate end-token entry preserved
        }
